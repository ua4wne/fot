...
class SaleController extends Controller
{
    public function create(Request $request){
        if($request->isMethod('post')){
            ...
            if(!Role::granted('sale_doc_add')){
                $msg = 'Попытка создания документа реализации!'. $model->doc_num;
                //вызываем event
            ...
            if($model->save()) {
                $msg = 'Добавлена новая реализация. Документ №'. $model->doc_num .' ID документа '.$model->id;
                //вызываем event
    ...
    public function deletePosition(Request $request){
        if($request->isMethod('post')){
            $id = $request->input('id');
            $model = SaleTable::find($id);
            if(!Role::granted('sale_doc_del')){
                $msg = 'Попытка удаления позиции документа реализации '. $model->sale->doc_num;
                //вызываем event
                event(new AddEventLogs('access',Auth::id(),$msg));
                return 'NO';
            }
    ...
    public function edit(Request $request){
        if($request->isMethod('post')){
            ...
            if(!Role::granted('sale_doc_edit')){
                $msg = 'Попытка редактирования документа реализации №'. $input['doc_num'];
                //вызываем event
                event(new AddEventLogs('access',Auth::id(),$msg));
                return 'NO';
            }
            ...
            if(!empty($model)){
                ...
                $model->user_id = Auth::id();
                if($model->update()) {
                    $msg = 'Был отредактирован документ реализации №'. $input['doc_num'] .' ID документа '.$id;
                    //вызываем event
                    event(new AddEventLogs('info',Auth::id(),$msg));
                    return $model->id;
                }
    ...
    public function delete(Request $request){
        if($request->isMethod('post')){
            ...
            $model = Sale::find($id);
            if(!Role::granted('sale_doc_del')){
                $msg = 'Попытка удаления документа реализации '. $model->doc_num;
                //вызываем event
                event(new AddEventLogs('access',Auth::id(),$msg));
                return 'NO';
            }
            SaleTable::where(['sale_id'=>$id])->delete(); //удаляем связанные записи
            if($model->delete()) {
                $msg = 'Реализация '. $model->doc_num .' была удалена!';
                //вызываем event
                event(new AddEventLogs('info',Auth::id(),$msg));
                return 'OK';
            }
